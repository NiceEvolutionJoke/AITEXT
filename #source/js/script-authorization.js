...
document.querySelector('.authorization__title').addEventListener('click', () => { openLoginPopup() })
...
/* miniPopups = document.querySelectorAll('.pop-up__content');
miniPopupBtns = document.querySelectorAll('.pop-up__btn');
for (let i = 1; i < miniPopups.length; i++) {
    if (miniPopups[i]) {
        miniPopups[i].style.cssText += `diplay: none;`;
    };
    miniPopupBtns.addEventListener('click', e => {
        if (e.classList.contains('false')) {
            miniPopups[0].style.cssText += `diplay: none;`;
            miniPopups[i].style.cssText += `diplay: block;`;
        };
    });
}; */
